...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">Escolher Template</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Selecione um template de mensagem aprovado para sua transmissão.
        </p>
      </div>
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-border bg-card/50">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Crie um template em Configurações primeiro.</p>
        </div>
      ) : (
